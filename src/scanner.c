...
static bool scan_preproc_start(TSLexer *lexer) {
  if (consume_char('#', lexer) && consume_char('#', lexer)) {
    lexer->mark_end(lexer);

    if (scan_block_start(lexer)) {
      lexer->mark_end(lexer);
      lexer->result_symbol = PREPROC_START;
      return true;
    }
  }
...
static bool scan_preproc_content(TSLexer *lexer) {
  if (ending_char == 0) { 
    if (scan_block_content(lexer)) {
      lexer->result_symbol = PREPROC_CONTENT;
      return true;
    }
...
bool tree_sitter_nelua_external_scanner_scan(void *payload, TSLexer *lexer,
  ...
  if (valid_symbols[PREPROC_END] && ending_char == 0 &&
      scan_block_end(lexer)) {
    reset_state();
  ...
  if (valid_symbols[PREPROC_CONTENT] && scan_preproc_content(lexer)) {
    return true;
  }
  ...
  if (valid_symbols[PREPROC_START]) {
    if (scan_preproc_start(lexer)) {
      return true;
    }
  }
